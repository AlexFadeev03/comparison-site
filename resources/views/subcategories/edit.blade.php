...
<div class="max-w-lg mx-auto px-4 py-6">
    ...
    <div class="bg-white rounded shadow p-6">
        <h1 class="text-xl font-bold mb-4">Edit Subcategory</h1>
        @if ($errors->any())
            ...
            <div class="mb-4">
                ...
                <select name="category_id" required class="w-full border rounded px-3 py-2">
                    ...
                    @foreach($categories as $cat)
                        <option value="{{ $cat->id }}" @if(old('category_id', $subcategory->category_id) == $cat->id) selected @endif>{{ $cat->name }}</option>
                    @endforeach
                </select>
            </div>
            <div class="mb-4">
                <label class="block text-gray-700 mb-1">Name</label>
                <input type="text" name="name" value="{{ old('name', $subcategory->name) }}" required class="w-full border rounded px-3 py-2 focus:outline-none focus:ring" />
            </div>
            <div class="mb-4">
                <label class="block text-gray-700 mb-1">Description</label>
                <textarea name="description" rows="2" class="w-full border rounded px-3 py-2">{{ old('description', $subcategory->description) }}</textarea>
            </div>
